...
<style type="text/css">
	...
	.wrapper .pager{border-top:1px #eee solid;padding-top:15px;}
	.wrapper .pager .page-div{display: inline-block;width:110px;}
	.wrapper .homePage,.wrapper .prePage,.wrapper .page-go,.wrapper .nextPage,.lastPage{border-radius:15px;color:#52de92;}
	.wrapper #page-input{display:inline-block;width:60px;border-radius: 10px;}
	.wrapper .bookImg{width:50px;}
	.wrapper .funbtn{margin:10px 0;}
	.wrapper .funbtn a{margin-right:10px;}
</style>
</head>
<body>
		...
		<div class="wrapper">
			<div class="main container">
												...
												<c:when test="${vs.first}">
													...
													<td style="border:0;border-left:1px solid #ddd;">
														...
														<c:if test="${order.orderStatus eq 2}">
															<p>已发货</p>
															<a href="OrderServlet?action=ship&id=${order.orderId}" class="btn btn-sm btn-success" onclick="return confirm('确认收货了吗？')">确认收货</a>
														</c:if>
				...
				<ul class="pager row">
					...
					<li>共 ${pageBean.pageCount} 页 | 当前 ${pageBean.curPage} 页</li>
					<li>
						跳转到
						<div class="input-group form-group page-div">
							<input id="page-input" class="form-control input-sm" type="text" name="page"/>
							<span><button class="page-go btn btn-sm btn-default">GO</button></span>
						</div>
					</li>
